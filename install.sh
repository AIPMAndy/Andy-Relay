...
SOURCE_DIR="$SCRIPT_DIR/skills/andy-relay"

# Detect environment and set appropriate target
if [ -n "${CODEX_HOME:-}" ]; then
  TARGET_ROOT="${CODEX_HOME}/skills"
  TARGET_DIR="$TARGET_ROOT/andy-relay"
  ENV_NAME="Codex"
...
if [ ! -f "$SOURCE_DIR/SKILL.md" ]; then
  echo "Andy-Relay skill not found at $SOURCE_DIR" >&2
  exit 1
fi
...
if [ -L "$TARGET_DIR" ] && [ "$(readlink "$TARGET_DIR")" = "$SOURCE_DIR" ]; then
  echo "Andy-Relay is already installed at $TARGET_DIR"
  exit 0
fi
...
echo "✓ Installed Andy-Relay for $ENV_NAME at $TARGET_DIR"
echo ""
echo "Usage:"
if [ "$ENV_NAME" = "Codex" ]; then
  echo "  Start a new Codex task and use: \$andy-relay"
else
  echo "  In Claude Code, use: /andy-relay"
fi
echo ""
